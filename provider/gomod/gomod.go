// Package gomod implements the permits provider for the Go module ecosystem: a
// Scanner for go.sum and a Fetcher that reads raw license text from the local
// module cache, falling back to the Go module proxy when the module is not
// cached.
package gomod
...
// Scan parses go.sum into a deduplicated dependency set. go.sum already lists
// the full resolved (incl. transitive) module set; the "/go.mod" hash lines are
// folded into the same (module,version) entry.
...
// ScanDirect parses only direct dependencies, implementing
// provider.DirectScanner. go.sum cannot distinguish direct from indirect, so it
// reads the sibling go.mod and returns its require directives that are not
// marked "// indirect".
...
// NewFetcher returns a [Fetcher]. proxyOverride, when non-empty, replaces the
// GOPROXY-derived list (comma or pipe separated).
...
// Fetch returns license artifacts, trying the local cache first and the module
// proxy as a fallback. An empty result with nil error means no license file.
...
// fromCache returns nil (not empty slice) when the module is not cached, so the
// caller knows to fall back to the proxy.
func (f *Fetcher) fromCache(dep model.Dependency, escPath, escVer string) ([]model.LicenseArtifact, error) {
	...
	if artifacts == nil {
		// Module is cached but has no license file: return empty (not nil) so
		// we do not redundantly hit the proxy.
