// Package output writes a collection run to disk: a machine-readable
// summary.json plus one Markdown file per license artifact, laid out as
// <ecosystem>/<name>/<version>/<spdx-or-original>.md with YAML frontmatter
...
func Write(dir string, summary model.Summary) error {
	...
	// Track filenames already used within each dependency directory so that
	// collisions (same SPDX id, or repeated original names) get a numeric
...
// writeArtifact writes one Markdown file and returns its slash-separated path
// relative to dir (the directory that also holds summary.json).
...
// baseName is the file stem (".md" is added later): the SPDX id when exactly
// one was detected, otherwise the original in-package filename with any
// license-ish extension stripped so we don't produce e.g. "LICENSE.md.md".
...
// stripLicenseExt removes a trailing documentation extension. It only strips a
// known set (not [filepath.Ext]) so SPDX-ish names like "Apache-2.0" or
// "BSD-3-Clause" keep their ".0"/"Clause" tail.
...
// uniquePath joins stem+".md" under depDir, appending -2, -3, ... if that file
// was already written this run.
...
// nameToPath turns a package name into nested path segments, preserving the
// natural hierarchy of scoped npm names (@scope/pkg) and Go module paths
// (host.com/x/y) while keeping each segment filesystem-safe.
